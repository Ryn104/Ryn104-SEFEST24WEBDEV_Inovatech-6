...
function setupPagination(dataArray) {
  ...
  document.querySelector('#sort-magnitude').addEventListener('click', sortMagnitude);
}
...
function sortMagnitude() {
  ...
  data.sort((a, b) => {
      if (!a.magnitudo) return 1;
      if (!b.magnitudo) return -1;
      return sortOrder === 'asc' ? a.magnitudo - b.magnitudo : b.magnitudo - a.magnitudo;
  });
  tampilkanData(data, 1);
